...
int main(int argc, char** argv)
{
    ...
    if (use_llvm)
    {
        LLVMCodegen llvm_gen = LLVMCodegen_init((&allocator), parser.stmt_pool, parser.expr_pool, parser.pattern_pool, (&resolver));
        ...
        const char* cmd = concatAlloc(concatAlloc(concatAlloc(concatAlloc(concatAlloc(concatAlloc("clang ", obj_file), " "), exe_lib_flag), " -o "), bin_name), " -lLLVM");
        {
...
ArrayList_Str Parser_parse_generic_params(Parser* self)
{
    ...
    if (Parser_peek(self, 0LL).tok_type == TokenType_LT)
    {
        ...
        while (!done)
        {
            ...
            {
                ArrayList_Str_push(&(type_params), concatAlloc(concatAlloc(tp_name, ":"), tv_get_str(Parser_expect(self, TokenType_IDENTIFIER).value)));
            } else
            {
...
PtrRefResult TypeChecker_parse_ptr_ref(TypeChecker* self, const char* type_str)
{
    ...
    } else if ((strlen(inner) > 1LL) && (inner[0LL] == ((char)(42LL))))
    {
        is_ptr = true;
        inner = substring(inner, 1LL, strlen(inner));
    } else if ((strlen(inner) > 5LL) && (strcmp(substring(inner, 0LL, 5LL), "&mut ") == 0LL))
    {
        is_ptr = true;
        is_mut = true;
        inner = substring(inner, 5LL, strlen(inner));
            ...
            if (pat.kind == PatternKind_pk_variant)
            {
                ...
                if (enum_idx != (-1LL))
                {
                    ...
                    while (vi < ArrayList_Variant_length(&(enum_stmt.enum_variants)))
                    {
                        ...
                        bool is_match = false;
                        if (strcmp(v.vname, pat.variant_name) == 0LL)
                        {
                            is_match = true;
                        } else
                        {
                            ...
                            if (pat_len > suffix_len)
                            {
                                ...
                                if (strcmp(actual_suffix, expected_suffix) == 0LL)
                                {
                                    is_match = true;
                                }
                            }
                ...
                while (bi < ArrayList_Str_length(&(pat.bindings)))
                {
                    ...
                    TypeChecker_define_symbol(self, ArrayList_Str_get(&(pat.bindings), bi), ptype, false, "var");
                    bi = (bi + 1LL);
                }
            }
...
void TypeChecker_check_expr(TypeChecker* self, int64_t expr_idx)
{
    ...
    if (expr.kind == ExprKind_ek_binary_op)
    {
        ...
        } else if ((((strcmp(op, "-") == 0LL) || (strcmp(op, "*") == 0LL)) || (strcmp(op, "/") == 0LL)) || (strcmp(op, "%") == 0LL))
        {
        ...
        } else if ((strcmp(op, "&&") == 0LL) || (strcmp(op, "||") == 0LL))
        {
        ...
        } else if (((((strcmp(op, "|") == 0LL) || (strcmp(op, "^") == 0LL)) || (strcmp(op, "&") == 0LL)) || (strcmp(op, "<<") == 0LL)) || (strcmp(op, ">>") == 0LL))
        {
        ...
        } else if ((((((strcmp(op, "==") == 0LL) || (strcmp(op, "!=") == 0LL)) || (strcmp(op, "<") == 0LL)) || (strcmp(op, ">") == 0LL)) || (strcmp(op, "<=") == 0LL)) || (strcmp(op, ">=") == 0LL))
        {
    ...
    if (expr.kind == ExprKind_ek_method_call)
    {
        ...
        } else if ((strcmp(recv_type, "Void") != 0LL) && (!TypeChecker_is_imported_name(self, meth_name)))
        {
        ...
        if (dot_pos >= 0LL)
        {
            ...
            while (fi < ArrayList_FieldInit_length(&(expr.struct_fields)))
            {
                ...
                while (pfi < ArrayList_Param_length(&(matched_var.params)))
                {
                    ...
                    if (strcmp(param.name, f.name) == 0LL)
                    {
                        found = true;
...
ImportResolver ImportResolver_init(KaiAllocator* allocator)
{
    ...
    self.import_stack = ArrayList_Str_init(allocator);    return self;
}void ImportResolver_err_at(ImportResolver* self, const char* msg, int64_t stmt_idx)
{
    if (!self->has_error)
    {
...
void ImportResolver_resolve_import(ImportResolver* self, int64_t stmt_idx, ArrayList_StmtNode* stmt_pool, ArrayList_ExprNode* expr_pool, ArrayList_PatternNode* pattern_pool)
{
    ...
    {
        if (strcmp(ArrayList_Str_get(&(self->import_stack), si), module_key) == 0LL)
        {
    ...
    if (expr.kind == ExprKind_ek_literal)
    {
        ...
        if (strcmp(vkind, "STRING") == 0LL)
        {
            if (expr.lit_value.tag == TokenValue_tv_str_TAG)
            {
                const char* v = expr.lit_value.tv_str.v;
                {
                    return concatAlloc(concatAlloc("\"", Codegen_escape_string(self, v)), "\"");
                }
            } else
            {
                return "\"\"";
    ...
    if (expr.kind == ExprKind_ek_str_interp)
    {
        ...
        while (i < ArrayList_StrInterpPart_length(&(expr.interp_parts)))
        {
            ...
            } else
            {
                ...
                } else if (strcmp(expr_type, "Str") == 0LL)
                {
                    part_str = expr_val;
                } else
                {
...
const char* Codegen_gen_stmt(Codegen* self, int64_t stmt_idx)
{
    ...
    if (stmt.kind == StmtKind_sk_impl_block)
    {
        ...
        while (idx < ArrayList_Int_length(&(stmt.impl_methods)))
        {
            ...
            bool is_init = ((((strcmp(method_name, "init") == 0LL) || (strcmp(method_node.func_return_type, struct_name) == 0LL)) || (strcmp(method_node.func_return_type, concatAlloc("*", struct_name)) == 0LL)) || (strcmp(method_node.func_return_type, concatAlloc("&", struct_name)) == 0LL));
            const char* ret_type = "";
            if (is_init)
            {
    ...
    if (stmt.kind == StmtKind_sk_return)
    {
        ...
        if (stmt.return_value >= 0LL)
        {
            ...
            if (ret_excl_pos >= 0LL)
            {
                ...
                } else if (strcmp(val_payload_type, "Void") == 0LL)
                {
        ...
        while (case_idx < ArrayList_MatchCase_length(&(stmt.match_cases)))
        {
            ...
            if (pat_node.kind == PatternKind_pk_literal)
            {
                ...
                } else if (pat_node.lit_value.tag == TokenValue_tv_str_TAG)
                {
                    ...
                    {
                        lit_str = concatAlloc(concatAlloc("\"", Codegen_escape_string(self, v)), "\"");
                    }
...
const char* Codegen_generate(Codegen* self, int64_t top_stmt_idx)
{
    ...
    while (ci < ArrayList_Str_length(&(self->import_resolver->cimport_headers)))
    {
        ...
        {
            is_baseline = true;
        } else if (strcmp(hdr, "stdbool.h") == 0LL)
        {
            is_baseline = true;
        } else if (strcmp(hdr, "stdio.h") == 0LL)
        {
            is_baseline = true;
        } else if (strcmp(hdr, "stdlib.h") == 0LL)
        {
            is_baseline = true;
        } else if (strcmp(hdr, "string.h") == 0LL)
        {
            is_baseline = true;
        } else if (strcmp(hdr, "sys/mman.h") == 0LL)
        {
            is_baseline = true;
        } else if (strcmp(hdr, "ctype.h") == 0LL)
        {
            is_baseline = true;
        }
        if (!is_baseline)
...
const char* CodegenBuilder_get_expr_type(CodegenBuilder* self, int64_t expr_idx)
{
    ...
    if (expr.kind == ExprKind_ek_field_access)
    {
        ...
        while (!done_strip)
        {
            ...
            } else if ((strlen(clean_base) > 5LL) && (strcmp(__kai_str_sub(clean_base, 0LL, 5LL), "*mut ") == 0LL))
            {
...
const char* CodegenBuilder_map_type(CodegenBuilder* self, const char* resolved_type)
{
    ...
    while (changed)
    {
        ...
        } else if ((strlen(inner) > 0LL) && (inner[0LL] == ((char)(38LL))))
        {
            ...
            inner = __kai_str_sub(inner, 1LL, strlen(inner));
            changed = true;
        } else if ((strlen(inner) >= 2LL) && (strcmp(__kai_str_sub(inner, 0LL, 2LL), "[]") == 0LL))
        {
            ptr_count = (ptr_count + 1LL);
            inner = __kai_str_sub(inner, 2LL, strlen(inner));
            changed = true;
        } else if ((strlen(inner) > 5LL) && (strcmp(__kai_str_sub(inner, 0LL, 5LL), "*mut ") == 0LL))
        {
            ptr_count = (ptr_count + 1LL);
    ...
    {
        base = "int64_t";
    } else if (strcmp(base, "i8") == 0LL)
    {
        base = "int8_t";
    } else if (strcmp(base, "i16") == 0LL)
    {
        base = "int16_t";
    } else if (strcmp(base, "i32") == 0LL)
    {
        base = "int32_t";
    } else if (strcmp(base, "u8") == 0LL)
    {
        base = "uint8_t";
    } else if (strcmp(base, "u16") == 0LL)
    {
        base = "uint16_t";
    } else if (strcmp(base, "u32") == 0LL)
    {
        base = "uint32_t";
    } else if (strcmp(base, "u64") == 0LL)
    {
        base = "uint64_t";
    } else if (strcmp(base, "usize") == 0LL)
    {
        base = "size_t";
    } else if (strcmp(base, "isize") == 0LL)
    {
        base = "intptr_t";
    } else if (strcmp(base, "Void") == 0LL)
    {
        base = "void";
    } else if (strcmp(base, "Bool") == 0LL)
    {
        base = "bool";
    } else if (strcmp(base, "Char") == 0LL)
    {
        base = "char";
    } else if (strcmp(base, "Float") == 0LL)
    {
        base = "double";
    } else if (strcmp(base, "Str") == 0LL)
    {
        base = "const char*";
    } else if (strcmp(base, "NoneType") == 0LL)
    {
        base = "void";
    } else if (strcmp(base, "TokenValue") == 0LL)
    {
        base = "TokenValue";
    } else if (strcmp(base, "TokenType") == 0LL)
    {
        base = "TokenType";
...
int64_t CodegenBuilder_gen_expr(CodegenBuilder* self, int64_t expr_idx)
{
    ...
    if (expr.kind == ExprKind_ek_literal)
    {
        ...
        if (strcmp(vkind, "STRING") == 0LL)
        {
            ...
            if (expr.lit_value.tag == TokenValue_tv_str_TAG)
            {
                const char* v = expr.lit_value.tv_str.v;
                {
                    val_str = concatAlloc(concatAlloc("\"", CodegenBuilder_cgb_replace_format_specifiers(self, CodegenBuilder_escape_string(self, v))), "\"");
                }
            } else
            {
            }
    ...
    if (expr.kind == ExprKind_ek_func_call)
    {
        ...
        if ((((strcmp(name, "Int") == 0LL) || (strcmp(name, "Float") == 0LL)) || (strcmp(name, "Bool") == 0LL)) || (strcmp(name, "Char") == 0LL))
        {
            if (ArrayList_Int_length(&(expr.func_args)) >= 1LL)
            {
                ...
                {
                    ctype_str = "double";
                } else if (strcmp(name, "Bool") == 0LL)
                {
                    ctype_str = "bool";
                } else if (strcmp(name, "Char") == 0LL)
                {
                    ctype_str = "char";
                }
                const char* src_type = CodegenBuilder_get_expr_type(self, ArrayList_Int_get(&(expr.func_args), 0LL));
    ...
    if (expr.kind == ExprKind_ek_field_access)
    {
        ...
        {
            is_arrow = true;
        } else if ((strlen(base_type) > 4LL) && (strcmp(__kai_str_sub(base_type, 0LL, 4LL), "*mut ") == 0LL))
        {
            is_arrow = true;
        }
        if (is_arrow)
        ...
        while (case_idx < ArrayList_MatchCase_length(&(stmt.match_cases)))
        {
            ...
            if (pat_node.kind == PatternKind_pk_literal)
            {
                ...
                } else if (pat_node.lit_value.tag == TokenValue_tv_str_TAG)
                {
                    const char* v = pat_node.lit_value.tv_str.v;
                    {
                        lit_str = concatAlloc(concatAlloc("\"", CodegenBuilder_escape_string(self, v)), "\"");
                    }
                } else if (pat_node.lit_value.tag == TokenValue_tv_bool_TAG)
                {
                    bool v = pat_node.lit_value.tv_bool.v;
        ...
        if (stmt.kind == StmtKind_sk_impl_block)
        {
            ...
            if ((cgb_map_find((&self->generic_struct_decls), struct_name) < 0LL) && (cgb_map_find((&self->generic_enum_decls), struct_name) < 0LL))
            {
                ...
                while (j < ArrayList_Int_length(&(stmt.impl_methods)))
                {
                    ...
                    StmtNode m_node = ArrayList_StmtNode_get(self->stmt_pool, m_idx);
                    const char* method_name = m_node.func_name;
                    bool is_init = ((((strcmp(method_name, "init") == 0LL) || (strcmp(m_node.func_return_type, struct_name) == 0LL)) || (strcmp(m_node.func_return_type, concatAlloc("*", struct_name)) == 0LL)) || (strcmp(m_node.func_return_type, concatAlloc("&", struct_name)) == 0LL));
...
LLVMCodegen LLVMCodegen_init(KaiAllocator* allocator, ArrayList_StmtNode* stmt_pool, ArrayList_ExprNode* expr_pool, ArrayList_PatternNode* pattern_pool, ImportResolver* import_resolver)
{
    ...
    self.local_alloca_types = ArrayList_Int_init(allocator);    return self;
}void LLVMCodegen_build_func_types(LLVMCodegen* self)
{
    int64_t i = 0LL;
    while (i < ArrayList_StmtNode_length(self->stmt_pool))
...
const char* LLVMCodegen_get_func_return_type(LLVMCodegen* self, const char* name)
{
    ...
    {
        if (strcmp(ArrayList_Str_get(&(self->func_type_names), i), name) == 0LL)
        {
...
void* LLVMCodegen_lookup_or_create_func(LLVMCodegen* self, const char* name)
{
    ...
    {
        if (strcmp(ArrayList_Str_get(&(self->func_cache_names), i), name) == 0LL)
        {
    ...
    void* llvm_ret = LLVMCodegen_map_type(self, ret_type_str);
    int64_t param_count = LLVMCodegen_get_func_param_count(self, name);
    ...
    } else if (param_count == 1LL)
    {
        void* ptype = LLVMCodegen_map_type(self, "*Char");
        fn_type = kai_func_type_1(llvm_ret, ptype, is_vararg);
    } else if (param_count == 2LL)
    {
        void* ptype0 = LLVMCodegen_map_type(self, "*Char");
        void* ptype1 = LLVMCodegen_map_type(self, "Int");
        fn_type = kai_func_type_2(llvm_ret, ptype0, ptype1, is_vararg);
    } else
    {
        fn_type = kai_func_type_1(llvm_ret, self->ptr_type, is_vararg);
    }
...
void* LLVMCodegen_map_type(LLVMCodegen* self, const char* type_name)
{
    if ((strcmp(type_name, "Int") == 0LL) || (strcmp(type_name, "Int64") == 0LL))
    {
        return self->int64_type;
    }
    if (strcmp(type_name, "Int32") == 0LL)
    {
    ...
    if (strcmp(type_name, "Double") == 0LL)
    {
...
void* LLVMCodegen_gen_expr(LLVMCodegen* self, int64_t expr_idx)
{
    ...
    if (expr.kind == ExprKind_ek_literal)
    {
        if (strcmp(expr.lit_vkind, "INT") == 0LL)
        {
            ...
            } else if (expr.lit_value.tag == TokenValue_tv_str_TAG)
            {
                const char* v = expr.lit_value.tv_str.v;
                {
                    val = LLVMCodegen_str_to_int(self, v);
                }
            } else
            {
            }
        ...
        if (strcmp(expr.lit_vkind, "STR") == 0LL)
        {
            const char* sval = "";
            if (expr.lit_value.tag == TokenValue_tv_str_TAG)
            {
            ...
            return LLVMBuildGlobalStringPtr(self->builder, sval, gname);
        }
    ...
    if (expr.kind == ExprKind_ek_identifier)
    {
        void* alloca = LLVMCodegen_find_alloca(self, expr.ident_name);
        if (alloca != ((void*)(((unsigned long long)(0LL)))))
        {
            void* elem_ty = LLVMCodegen_find_alloca_type(self, expr.ident_name);
    ...
    if (expr.kind == ExprKind_ek_func_call)
    {
        ...
        void* fn_ptr_ty = LLVMTypeOf(fn_val);
        void* fn_ty = LLVMGetElementType(fn_ptr_ty);
        void* fn_ret_ty = LLVMGetReturnType(fn_ty);
        int64_t arg_count = ArrayList_Int_length(&(expr.func_args));
        if (arg_count == 0LL)
        {
            (void)(LLVMBuildCall2(self->builder, fn_ty, fn_val, ((void*)(((unsigned long long)(0LL)))), 0LL, ""));
            return ((void*)(((unsigned long long)(0LL))));
        }
        void* arg0 = LLVMCodegen_gen_expr(self, ArrayList_Int_get(&(expr.func_args), 0LL));
        if (arg_count == 1LL)
        {
            return kai_build_call_1(self->builder, fn_ty, fn_val, arg0, "");
        }
        void* arg1 = LLVMCodegen_gen_expr(self, ArrayList_Int_get(&(expr.func_args), 1LL));
        if (arg_count == 2LL)
        {
            return kai_build_call_2(self->builder, fn_ty, fn_val, arg0, arg1, "");
        }
        void* arg2 = LLVMCodegen_gen_expr(self, ArrayList_Int_get(&(expr.func_args), 2LL));
        if (arg_count == 3LL)
        {
            return kai_build_call_3(self->builder, fn_ty, fn_val, arg0, arg1, arg2, "");
        }
        void* arg3 = LLVMCodegen_gen_expr(self, ArrayList_Int_get(&(expr.func_args), 3LL));
        if (arg_count == 4LL)
        {
...
bool LLVMCodegen_emit_object(LLVMCodegen* self, const char* obj_path)
{
    ...
    LLVMVerifyModule(self->module, LLVMPrintMessageAction, (&self->tmp_err));
                ...
                {
                    {
                        ...
                        if (((!is_flag) && (strcmp(a, patch_file) != 0LL)) && (strcmp(a, "addParam") != 0LL))
                        {
                            ...
                            {
                                fn_name = a;
                            } else if (strcmp(param_name, "") == 0LL)
                            {
                                param_name = a;
                            } else if (strcmp(param_type, "") == 0LL)
                            {
                                param_type = a;
                            }
                        }
